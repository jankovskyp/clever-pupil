...
import { Home, Plus, Trash2, Languages, Loader2, Calendar, Volume2 } from 'lucide-react';
...
export default function SettingsPage() {
  ...
  const handleDelete = async (id: string, audioUrl?: string) => {
    ...
    if (confirm('Opravdu chceš smazat toto slovíčko?')) {
      ...
      // 2. Try to delete audio file if exists
      if (audioUrl) {
        try {
    ...
    <main className="h-screen w-screen bg-slate-50 flex flex-col p-6 font-sans text-slate-900 overflow-hidden">
      ...
      <div className="flex gap-8 flex-1 min-h-0">
        ...
        <div className="w-[400px] bg-white p-8 rounded-[3rem] shadow-xl border-4 border-slate-100 flex flex-col gap-6 shrink-0">
          ...
          <div className="flex flex-col gap-2 text-board-black">
            ...
            <div className="flex gap-2">
              ...
              <button 
                ...
                className="bg-slate-800 text-white p-4 rounded-2xl shadow-lg hover:scale-95 transition-all disabled:opacity-30"
              >
          ...
          <div className="mt-auto pt-6 border-t-2 border-slate-50 text-board-black">
            <DeskButton 
              size="lg" 
              variant="outline"
              className="w-full py-6 text-2xl border-slate-800 text-slate-800 bg-white" 
              onClick={handleAdd} 
              disabled={!enWord || !czWord || isLoading || !isSupabaseConfigured}
            >
              {isLoading ? <Loader2 className="w-8 h-8 animate-spin" /> : <Plus className="w-8 h-8 mr-2" />} 
              {isLoading ? 'Zpracovávám...' : 'Přidat slovíčko'}
            </DeskButton>
            <p className="text-[10px] text-slate-400 text-center mt-4 uppercase font-bold">Automaticky vytvoří zvukovou stopu</p>
          </div>
        </div>

        {/* Seznam slovíček */}
        <div className="flex-1 bg-white p-8 rounded-[3rem] shadow-xl border-4 border-slate-100 flex flex-col overflow-hidden text-slate-900">
           ...
           <div className="flex-1 overflow-y-auto pr-4 flex flex-col gap-3">
              {words.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center opacity-20 text-slate-900 font-sans">
                   <p className="text-2xl font-black uppercase">Zatím prázdno</p>
                </div>
              ) : (
                words.map((w) => (
                  <div key={w.id} className="flex justify-between items-center bg-slate-50 p-5 rounded-2xl border-2 border-slate-100 hover:border-slate-200 transition-all text-slate-900">
                    <div className="flex items-center gap-8 text-2xl font-black">
                      <div className="w-48">
                        <p className="text-slate-800 truncate leading-none uppercase">{w.en}</p>
                    ...
                    <div className="flex items-center gap-4">
                      {w.audio_url && (
                        <button onClick={() => playPreview(w.audio_url!)} className="text-[#38BDF8] hover:bg-white p-3 rounded-xl transition-all">
                          <Volume2 className="w-6 h-6" />
                        </button>
                      )}
                      <button onClick={() => handleDelete(w.id, w.audio_url)} className="text-slate-200 hover:text-error transition-colors p-3 rounded-xl">
